...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="space-y-8 sm:space-y-16 mt-8 sm:mt-16 text-[#F2F2F0]">
            <div className="flex flex-col items-center font-[Space_Grotesk]">
                <div className="mx-auto"><Toaster /></div>
                {(cacheResult.data &&
                    <ol className="w-1/2 mx-auto font-[Darker_Grotesque] font-medium tracking-wide text-gray-300 text-lg sm:text-xl md:text-2xl">
                        <li>Country: <span className="font-light">{cacheResult.data.location.country}</span></li>
                        <li className="overflow-auto whitespace-nowrap ">Address: <span className="font-light">{cacheResult.data.location.address}</span></li>
